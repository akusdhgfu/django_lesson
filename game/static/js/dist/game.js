...
<div class="ac_game_menu">
    <div class="ac_game_menu_field">
        <div class="ac_game_menu_field_item ac_game_menu_field_item_singal_mode">
            单人模式
        </div>
        <br>
...
`);
        ...
        this.$single_mode = this.$menu.find('.ac_game_menu_field_item_singal_mode');
...
// requestAnimationFrame函数能保证回调函数在屏幕每一次的刷新间隔中只被执行一次，一般是60次/s，但不同浏览器可能不一样
requestAnimationFrame(AC_GAME_ANIMATION);
...
class Player extends AcGameObject{
    ...
    update(){
        this.safe_time += this.timedelta / 1000;
        if(!this.is_me && this.safe_time > 5 && Math.random() < 1 / 180.0) {
...
`);
    ...
    get_random_color() {
        let colors = ["green", "bule", "red", "pink", "grey"];
